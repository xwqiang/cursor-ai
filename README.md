...
npm install
cp .env.example .env
# 编辑 .env：填入 CURSOR_API_KEY、TELEGRAM_BOT_TOKEN、PROJECT_GIT_URL（推荐）
```

> **服务器部署**：只需 clone 本仓库，在 `.env` 填 `PROJECT_GIT_URL`，Bot 首次启动会自动 `git clone` 到 `data/repos/<id>/`。无需事先 checkout 业务仓库，也不要拷贝本机 Mac 路径。
...
本地开发若已有 checkout，可改用 `PROJECT_ROOT=/path/to/project`，或 `./start.sh --project /path/to/project`。
...
项目配置见 `data/projects.json`（单项目首次启动自动生成；多项目见 `data/projects.json.example`）。群与项目的绑定由 Bot 在 `/projects` 切换时自动写入 `data/chat-projects.json`，无需手改。详见 [docs/projects.md](docs/projects.md)。
...
| `PROJECT_GIT_URL` | ✅* | — | 目标 Git 仓库 URL（服务器部署推荐） |
| `PROJECT_GIT_BRANCH` | ❌ | `main` | Git 默认分支 |
| `PROJECT_ID` | ❌ | 从 URL 推导 | 项目 ID，用于 `data/repos/<id>/` |
| `PROJECT_ROOT` | ✅* | — | 本地项目根目录（开发机可选） |
| `PROJECT_NAME` | ❌ | 目录名 / ID | 项目名称，注入系统提示 |
...
│   ├── projects.json.example           # 多项目配置模板
│   ├── tasks.db                        # 定时任务持久化（SQLite）
│   ├── projects.json                   # 项目注册表（运行时）
│   ├── chat-projects.json              # 群→项目绑定（Bot 自动写入，勿手改）
│   └── repos/                          # Git 项目克隆目录
├── docs/
