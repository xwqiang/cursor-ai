...
npm install
cp .env.example .env
# 编辑 .env：填入 CURSOR_API_KEY、TELEGRAM_BOT_TOKEN、PROJECT_ROOT（目标项目在**当前机器**上的绝对路径）
```

> **换机器 / 服务器部署**：`.env` 和 `data/projects.json` 不会随 git 提交。不要拷贝本机 Mac 路径；在服务器上重新设置 `PROJECT_ROOT`，或 `cp data/projects.json.example data/projects.json` 后改 `path`。
